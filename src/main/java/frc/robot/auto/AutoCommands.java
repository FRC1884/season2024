...
public class AutoCommands {
    public static void registerAutoCommands() {
        ...
        Supplier<Pose2d> getTarget = () -> DriverStation.getAlliance().isPresent() && DriverStation.getAlliance().get() == DriverStation.Alliance.Blue
                ? Coordinates.BLUE_SPEAKER : Coordinates.RED_SPEAKER;
        ...
        NamedCommands.registerCommand("SpoolShooter", shooter.setFlywheelVelocityCommand(
                () -> lookupTable.get(poseEstimator.getDistanceToPose(getTarget.get().getTranslation())).getRPM()
        ));

        NamedCommands.registerCommand("Pivot", pivot.updatePosition(
                () -> lookupTable.get(poseEstimator.getDistanceToPose(getTarget.get().getTranslation())).getAngleSetpoint()
        ));
        ...
        NamedCommands.registerCommand("VisionIntake", new IntakeUntilLoadedCommand().raceWith(Vision.getInstance().PIDtoNoteRobotRelativeCommand()));
