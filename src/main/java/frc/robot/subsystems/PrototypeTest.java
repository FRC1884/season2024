...
public class PrototypeTest extends SubsystemBase {
  ...
  private ShuffleboardLayout motor1Layout =
      Shuffleboard.getTab("Prototyping Tab")
          .getLayout("Motor 1", BuiltInLayouts.kList)
          .withSize(2, 2)
          .withProperties(Map.of("Label Position", "HIDDEN"))
          .withPosition(1, 1);

  private ShuffleboardLayout motor2Layout =
      Shuffleboard.getTab("Prototyping Tab")
          .getLayout("Motor 2", BuiltInLayouts.kList)
          .withSize(2, 2)
          .withProperties(Map.of("Label Position", "HIDDEN"))
          .withPosition(3, 1);

  private ShuffleboardLayout motor3Layout =
      Shuffleboard.getTab("Prototyping Tab")
          .getLayout("Motor 3", BuiltInLayouts.kList)
          .withSize(2, 2)
          .withProperties(Map.of("Label Position", "HIDDEN"))
          .withPosition(5, 1);

  private HashMap<CANSparkMax, ShuffleboardLayout> motorToLayout =
      new HashMap<CANSparkMax, ShuffleboardLayout>();

  private HashMap<CANSparkMax, HashMap<String, GenericEntry>> motorToEntry =
      new HashMap<CANSparkMax, HashMap<String, GenericEntry>>();
  ...
  /**
   * Checks if there is a change made on Shuffleboard to any of the motor values and changes it if
   * there is.
   *
  ...
  private void updateMotor(CANSparkMax motor) {
    ...
    for (String key : dataEntryKeys) {
      switch (key) {
        case "RunSpeed":
          if (motor.get() != entry.get(key).getDouble(0)) motor.set(entry.get(key).getDouble(0));
        case "P":
        ...
        case "maxVel":
          if (motor.getPIDController().getSmartMotionMaxVelocity(0)
              != entry.get(key).getDouble(2000))
        ...
        case "AllowErr":
          if (motor.getPIDController().getSmartMotionAllowedClosedLoopError(0)
              != entry.get(key).getDouble(0))
            motor
                .getPIDController()
                .setSmartMotionAllowedClosedLoopError(entry.get(key).getDouble(0), 0);
        case "PIDEnable":
          if (entry.get(key).getBoolean(false)) runMotorToSetpoint(motor);
      }
    }
  }
